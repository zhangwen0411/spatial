...
object TriangleCounting extends SpatialAppCompiler with TriangleCountingApp
trait TriangleCountingApp extends SpatialApp {
  ...
  //  3. Intersection in Spatial of two sets is O(M*N) where M and N are sizes of two sets
  def main() {
    ...
    def commonNeighbors(n1:Rep[Index], n2:Rep[Index], vl:Rep[ForgeArray[Index]], el:Rep[ForgeArray[Index]]):Rep[Index] = {
      ...
      if (n1size==0.as[Index] || n2size==0.as[Index]) {
        ...
        var n1cur = n1pt
        var n2cur = n2pt
        //Can use a while loop here but try to be consistent with Spatial
