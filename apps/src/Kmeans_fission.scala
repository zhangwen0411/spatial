...
object Kmeans_fission extends SpatialAppCompiler with KmeansApp_fission
trait KmeansApp_fission extends SpatialApp {
        ...
        Pipe(N by BN par param(1)){i =>
          ...
          Fold(BN par P3, PR)(newCents, 0.as[T]){pt =>
            ...
            Pipe(K by 1, D par P2){(ct,d) =>
              ...
              localCent(ct, d) = mux(ct == minCent.value, elem, 0.as[T])
            }
            localCent
