...
testname="OuterProduct"
server="max" # max or londonvm
start_channel=7
file=${SPATIAL_HOME}/apps/src/OuterProduct.scala
staging_area=${HOME}/staging/${testname}

innerPar=(2 4 8) # 32 on max2
outerPar=(2)
tileSize1=(192)
tileSize2=(192)
...
for p1 in ${innerPar[@]}; do
      ...
      for p3 in ${tileSize1[@]}; do
        for p4 in ${tileSize2[@]}; do
    	    ...
    	    if [ $i -ge $startAt ]; then
		      	...
		      	sed -i "s/val tileSize1 = [0-9]\+/val tileSize1 = $p3/g" $file
		      	sed -i "s/val tileSize2 = [0-9]\+/val tileSize2 = $p4/g" $file
