...
start_channel=3
file=${SPATIAL_HOME}/apps/src/SMV.scala
staging_area=${HOME}/staging/${testname}

innerPar=(4 8) # 32 on max2
outerPar=(1)
NNZ=(60)
tileSize=(384 768)
pp=(3840)
        ...
        for p4 in ${NNZ[@]}; do
          for p5 in ${tileSize[@]}; do
    	    ...
    	    if [ $i -ge $startAt ]; then
		      	...
		      	sed -i "s/val pp = [0-9]\+/val pp = $p3/g" $file
		      	sed -i "s/val tileSize = [0-9]\+/val tileSize = $p5/g" $file
		      	sed -i "s/val NNZ = [0-9]\+/val NNZ = $p4/g" $file
