...
start_channel=1
file=${SPATIAL_HOME}/apps/src/GDA.scala
      ...
      for p3 in ${maxc[@]}; do
        for p4 in ${tileSize[@]}; do
    	    ...
    	    if [ $i -ge $startAt ]; then
		      	...
		      	sed -i "s/val MAXC = [0-9]\+/val MAXC = $p3/g" $file
		      	sed -i "s/val tileSize = [0-9]\+/val tileSize = $p4/g" $file
				...
				if [ "$wc" -ne 1 ]; then
					echo "Remake fucked up fastmake $p1 $p2 $mp $p3 $p4 $P!"
					exit 1
				fi
				...
				# Copy results file to out, only last one will have all data
		      	echo "$p1,$mp,$p2,$p3,$p4,$P,${server}${i}," >> $fname
				...
				eval "$cmd" 2>&1 > /dev/null
		      	echo "Finished $p1 $p2 $mp $p3 $p4 $P on channel $i..."

			else
				echo "Skipping $p1 $p2 $mp $p3 $p4 $P on channel $i..."
			fi
