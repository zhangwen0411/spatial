...
testname="LogReg"
server="londonvm" # max or londonvm
start_channel=1
file=${SPATIAL_HOME}/apps/src/LogReg.scala
staging_area=${HOME}/staging/${testname}

innerPar=(4 16) # 32 on max2
outerPar=(1 4)
tileSize=(384)
dim=(192)
      ...
      for p3 in ${tileSize[@]}; do
        for p4 in ${dim[@]}; do
    	    ...
    	    if [ $i -ge $startAt ]; then
		      	...
		      	echo "Running $p1 $p2 $d0 on channel $i..."
				...
				if [ "$wc" -ne 1 ]; then
					echo "Remake fucked up fastmake $p1 $p2 $mp $p3 $p4 $p5!"
					exit 1
				fi
				...
				# Copy results file to out, only last one will have all data
		      	echo "$p1,$mp,$p2,$p3,$p4,$p5,${server}${i}," >> $fname
				...
				eval "$cmd" 2>&1 > /dev/null
		      	echo "Finished $p1 $p2 $mp $p3 $p4 $p5 on channel $i..."

			else
				echo "Skipping $p1 $p2 $mp $p3 $p4 $p5 on channel $i..."
			fi
